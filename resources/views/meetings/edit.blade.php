@extends('layouts.app')
@section('title', __('Meetings.edit_meeting'))
...
<div class="cf-wrap">
  ...
  <div class="cf-header">
    <div>
      <h5 style="font-weight:700;margin:0 0 3px">{{ __('Meetings.edit_meeting') }}</h5>
      <span style="font-size:12px;color:#999">{{ __('Meetings.meetings') }} #{{ $meeting->id }}</span>
    </div>
    <a href="{{ route('meetings.show', $meeting->id) }}" class="btn-cancel">
      <i class="ti ti-arrow-left" style="font-size:14px"></i> {{ __('Meetings.view_meeting') }}
    </a>
  </div>
  ...
  <div class="cf-card">
    <form action="{{ route('meetings.update', $meeting->id) }}" method="POST">
      ...
      <p class="cf-section-title">
        <i class="ti ti-info-circle" style="font-size:13px;vertical-align:-1px;margin-right:4px"></i>
        {{ __('Meetings.basic_information') }}
      </p>
      <div class="cf-grid-2">
        ...
        <div class="cf-group">
          ...
          <input type="text" name="title"
                 ...
                 placeholder="{{ __('Meetings.enter_meeting_title') }}" required>
        ...
        <div class="cf-group">
          ...
          <input type="date" name="meeting_date"
                 value="{{ old('meeting_date', \Carbon\Carbon::parse($meeting->meeting_date)->format('Y-m-d')) }}" required>
        ...
        <div class="cf-group">
          <label>{{ __('Meetings.status') }}</label>
          <select name="status">
            @foreach(['scheduled','ongoing','completed','cancelled'] as $s)
              <option value="{{ $s }}" {{ old('status', $meeting->status) == $s ? 'selected' : '' }}>
                {{ __('Meetings.'.$s) }}
              </option>
            @endforeach
          </select>
      ...
      {{-- Details --}}
      <p class="cf-section-title">
        <i class="ti ti-file-text" style="font-size:13px;vertical-align:-1px;margin-right:4px"></i>
        {{ __('Meetings.details') }}
      </p>
      <div class="cf-grid-1">
        <div class="cf-group">
          ...
          <textarea name="agenda" rows="3"
                    placeholder="{{ __('Meetings.enter_agenda') }}">{{ old('agenda', $meeting->agenda) }}</textarea>
        </div>
        <div class="cf-group">
          <label>{{ __('Meetings.description') }}</label>
          <textarea name="description" rows="3"
                    placeholder="{{ __('Meetings.enter_description') }}">{{ old('description', $meeting->description) }}</textarea>
        </div>
      </div>
      ...
      {{-- Participants --}}
      <p class="cf-section-title">
        <i class="ti ti-users" style="font-size:13px;vertical-align:-1px;margin-right:4px"></i>
        {{ __('Meetings.participants') }}
      </p>
      <div class="cf-group">
        <label>{{ __('Meetings.select_participants') }}</label>
        @php
          $selected = $meeting->participants->map(fn($p) =>
            ($p->pivot->participant_type ?? 'user') === 'employee'
              ? 'employee_'.$p->id
              : 'user_'.$p->id
          )->toArray();
        @endphp
          ...
          <optgroup label="Admins">
            @foreach($users as $user)
              <option value="user_{{ $user->id }}"
                {{ in_array('user_'.$user->id, old('participants', $selected)) ? 'selected' : '' }}>
                {{ $user->name }}
              </option>
            @endforeach
          </optgroup>
          <optgroup label="Employees">
            @foreach($employees as $employee)
              <option value="employee_{{ $employee->id }}"
                {{ in_array('employee_'.$employee->id, old('participants', $selected)) ? 'selected' : '' }}>
                {{ $employee->name }}
              </option>
            @endforeach
          </optgroup>
