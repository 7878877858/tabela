@extends('layouts.app')
@section('title', __('farm.report_utility'))

@section('content')
@php $currency = \App\Models\Setting::get('currency', '₹'); @endphp
@include('reports.partials.date-filter')
<x-form-card :title="__('farm.report_utility')" icon="📋" :flush="true">
    <x-erp-listing :paginator="$records" :per-page="$perPage" :search="false">
        <x-responsive-table>
            <table class="ds-table">
                <thead><tr><th>{{ __('farm.bill_date') }}</th><th>{{ __('farm.bill_type') }}</th><th class="text-end">{{ __('farm.amount') }}</th><th>{{ __('farm.status') }}</th></tr></thead>
                <tbody>
                    @forelse($records as $row)
                    <tr>
                        <td>{{ $row->bill_date->format('d-m-Y') }}</td>
                        <td>{{ __('farm.' . $row->bill_type) }}</td>
                        <td class="text-end">{{ $currency }}{{ number_format($row->amount, 2) }}</td>
                        <td>{{ $row->status === 'paid' ? __('farm.paid') : __('farm.pending') }}</td>
                    </tr>
                    @empty
                    <tr><td colspan="4" class="text-center text-muted" style="padding:24px;">{{ __('farm.no_records') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </x-responsive-table>
    </x-erp-listing>
</x-form-card>
@endsection
