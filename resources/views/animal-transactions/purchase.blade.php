@extends('layouts.app')
@section('title', __('farm.purchase_animal'))

@section('content')

<x-section-header :title="__('farm.purchase_animal')" icon="🐃">
    <x-slot:actions>
...
<x-form-card :title="__('farm.purchase_animal')" icon="➕">
    <form method="POST" action="{{ route('animal-transactions.purchase.store') }}" class="grid-2">
        @csrf
        ...
        <div class="form-group">
            <label class="form-label">{{ __('farm.purchase_price') }} *</label>
            <input type="number" step="0.01" min="0" name="amount" class="form-control" value="{{ old('amount') }}" required>
        </div>
        <div class="form-group">
            <label class="form-label">{{ __('farm.seller') }}</label>
